module ActivitiesHelper 
  ...
  def track_tag(activity)
    ...
    div = content_tag("div",track_icon_tag(activity),{"class" => "btn", "data-method" => "#{method}", "data-type" => "#{controller_name}" , "data-id" => "#{activity.id}" , "id" => "track-item"})  
    div
  end

  def track_icon_tag(activity)
    class_content = tracked?(activity) ?  'fa fa-heart fa-2x' : 'fa fa-heart-o fa-2x'
    text = tracked?(activity) ? t(:untrack) : t(:track)
   "<i class='#{class_content}'> <span class='i-text'>#{text}</span></i>".html_safe
  end

  #create participate button 
  def participate_tag(activity)
    method = participate?(activity) ? "dropout" : "participate"
    div = content_tag("div",participate_icon_tag(activity),{"class" => "btn", "data-method" => "#{method}", "data-type" => "#{controller_name}" , "data-id" => "#{activity.id}" , "id" => "participate-item"})  
    div
  end

  def participate_icon_tag(activity)
    class_content = participate?(activity) ?  'fa fa-sign-in fa-2x' : 'fa fa-sign-out fa-2x'
    text = participate?(activity) ? t(:dropout) : t(:participate)
   "<i class='#{class_content}'> <span class='i-text'>#{text}</span></i>".html_safe
  end
